...
(* Select only the points that are inside the window -may have been randomly displaced outside box *)
...
PlotStyle->{{Thick,RGBColor[0.545,0.0,0.8]},{Dashed,RGBColor[0.1333,0.5451,0.1333]}},PlotLegends->{ namesO[[nf]](*<>"\n r="<>ToString[NumberForm[#[[1]],3]]<>If[latticeO\[Equal]"square",
...
colours=RegularPolygonColourScheme;
If[nfuncts>=Length@colours,Table[RandomColor[],{x,Range[nfuncts]}]];

polys=Table[Graphics[{
EdgeForm[{Black}],Directive[{colours[[x]],Opacity[0.5]}],Polygon[standardPolygon[40]]
}],{x,Range[nfuncts]}];
...
Joined->True,
Filling->Axis,
FillingStyle->Directive[{Opacity[0.3],colours[[nf]]}],
FrameLabel->{"","\[CapitalDelta]"},
RotateLabel->False,
FrameStyle->Thick,
...
ListPlot[
{pcfLatticeList[[nf]]+Table[{0,If[absgrO==False,(nf-1),maxR]},{a,Range@Length[pcfLatticeList[[nf]]]}],
pcfDataList[[nf]]+Table[{0,If[absgrO==False,(nf-1),maxR]},{a,Range@Length[pcfDataList[[nf]]]}]}
,
Joined->True,
Filling->{1-> {If[absgrO==False,(nf-1),maxR],Gray}, 2->{{1},{GrayLevel[0.7],Directive[{Opacity[0.4],White}]}},2-> {If[absgrO==False,(nf-1),maxR],Directive[{Opacity[0.7],colours[[nf]]}]}(*1\[Rule]{{2},{None,Red}}*)},
(*FillingStyle\[Rule]({Directive[{Opacity[0.4],colours[[nf]]}]}(*&/@{0.1,0.4}*)),*)
...
("Squaratic Disorder: \[Sigma]="<>ToString[NumberForm[#[[2]],3]]),("Hexatic Disorder: \[Sigma]="<>ToString[NumberForm[#[[2]],3]])]*)     )}&/@{Reverse[dataExpectedDiameterList][[nf]]},LegendMarkers-> Reverse@polys[[nfuncts-nf+1]],LegendMarkerSize->{{lmsize,lmsize}}],
...
If[latticeO==""||latticeO=="hex",latticeO="Hexatic"];
If[latticeO=="square",latticeO="Squaratic"];
Column[{returnedPlot
,
TableForm[
...
Options[\!\(TraditionalForm\`PlotVoronoiTessellations\)]= {type->"bop",symmetry->6,cells->{},typedata-> Null ,points->True,scalebar->False, stats->"Count", boundary->"NOEDGE",box->{} ,raster-> False};
SetOptions[PlotVoronoiTessellations,type->"bop",symmetry->6,cells->{},typedata-> Null ,points->True,scalebar->False, stats->"Count", boundary->"NOEDGE",box->{},raster->False ];
PlotVoronoiTessellations[data2Din_, OptionsPattern[] ]:=Block[
{data2D,npoints,vor,hexagonalSpacing,nnDist,hexagonalVoronoi,voronoiColour,polygonColour,polyAreaColourTable,returnVor,xmin,ymin, xmax,ymax,intensity,\[CapitalDelta],histstats,histtype,sym,bopdata,boptrue=False,scalebargraphic,r,\[Theta],r\[Theta],vorCenters,nvor,m,renderVor,extraGraphics,
...
voronoiColour=BondOrderColourScheme[#,symmetryO]&/@polyAreaColourTable[[;;Length[vor]]];
If[Max[polyAreaColourTable]>1,scalebargraphic=ScaleBars["over-bop",sym->symmetryO];,scalebargraphic=ScaleBars["bop",sym->symmetryO];];
boptrue=True;
...
If[boptrue==True,
AppendTo[returnVor,VoronoiHistogram[vor, type->typeO,stats->statsO,  sym->symmetryO,bopdata-> polyAreaColourTable]];,
...
Options[VoronoiHistogram]= {type->"vor",stats->"Probability",scalebar->True,sym->1,bopdata->Null};
SetOptions[VoronoiHistogram,type->"vor",stats->"Probability",scalebar->True,sym->1,bopdata->Null];
VoronoiHistogram[inVoronoiCells_, OptionsPattern[]]:=Block[{
typeO= OptionValue[type],
statsO= OptionValue[stats],
scalebarO= OptionValue[scalebar],
...
HistCoordColours= BondOrderColourScheme[#,symO]&/@(0.1(Range[12]-1));
hlist=HistogramList[bopO,{0.0,1.1,0.10},statsO];
maxh=Position[hlist,Max[hlist[[2,;;]]  ]][[1,2]];
mh=hlist[[2,maxh]]*1.15;
offset=Position[hlist[[2]],SelectFirst[hlist[[2]],#> 0.0&]][[1,1]];
If[statsO=="Probability",placedNumber=Placed[NumberForm[#,{5,3}],Above]&];
